...
|---|---|---|---|---|---|---|---|---|---|
| **Country** | ✓ full, fresh citations (new BR-16) | ✓ | ✓ `CountryCode` rewritten as a ZIO Prelude `Newtype` — BR-01 (shape) is now a real, enforced smart constructor, not dead code; BR-16 (ISO membership) is a separate, also-live check, now `CountryCode.isRecognized`/`validateIso` (pure, in-memory — originally `CountryRepository.validateCode` against a DB table, see `docs/analysis/validation-analysis-hexagonal.md` §2.2/§2.4a/§6) | ✓ Quill wired | ✓ full CRUD, validated; create rejects malformed codes (`CountryCode.make`) and non-ISO codes (`validateIso`), both → `InvalidCountryCode` (400) | ✓ both layers | ✓ domain (`CountryCodeSpec`) + both adapters | ✓ Postman "E2E — Country" folders | ✓ V1, V7; V12 (`country_codes` master table) dropped by V19, superseded by domain-layer check |
