...
import dev.cmartin.aerohex.domain.model.{Aircraft, Airline, Country, CountryCode, IcaoCode, Registration}
import dev.cmartin.aerohex.domain.port.out.{AircraftRepository, AirlineRepository, CountryRepository}
  ...
  def tests: List[Spec[AircraftRepository & AirlineRepository & CountryRepository, Any]] = List(
    test("saves and finds an aircraft by registration") {
      for
        ...
        ecMig   = Aircraft(Registration("EC-MIG"), "B788", "Boeing 787-8", IcaoCode("IBE"))
    ...
    test("findAll includes saved aircraft") {
      for
        ...
        repo <- ZIO.service[AircraftRepository]
        _    <- repo.save(Aircraft(Registration("F-GKXA"), "A388", "Airbus A380-800", IcaoCode("AFR")))
    ...
    test("update changes the type code, description, and airline of an existing aircraft") {
      for
        ...
        repo    <- ZIO.service[AircraftRepository]
        _       <- repo.save(Aircraft(Registration("CS-TUA"), "A319", "Airbus A319", IcaoCode("TAP")))
        updated  = Aircraft(Registration("CS-TUA"), "A320", "Airbus A320", IcaoCode("PGA"))
    ...
    test("update fails with AircraftNotFound for an unknown registration") {
      for
        ...
        repo  <- ZIO.service[AircraftRepository]
        error <- repo.update(Aircraft(Registration("ZZ-ZZZ"), "A320", "Nowhere", IcaoCode("LGL"))).flip
    ...
    test("update fails with AirlineNotFound when the new airline code does not exist") {
      for
        ...
        repo  <- ZIO.service[AircraftRepository]
        _     <- repo.save(Aircraft(Registration("OO-ABC"), "A320", "Airbus A320", IcaoCode("BEL")))
        error <- repo.update(Aircraft(Registration("OO-ABC"), "A320", "Airbus A320", IcaoCode("YYY"))).flip
      yield assertTrue(error == DomainError.AirlineNotFound("YYY"))
    },
    test("save fails with AirlineNotFound for an unknown airline code") {
      for
        repo  <- ZIO.service[AircraftRepository]
        error <- repo.save(Aircraft(Registration("N00000"), "B737", "Boeing 737", IcaoCode("XXX"))).flip
    ...
    test("save fails with AircraftAlreadyExists on a duplicate registration") {
      for
        ...
        repo  <- ZIO.service[AircraftRepository]
        _     <- repo.save(Aircraft(Registration("PH-BHA"), "B789", "Boeing 787-9", IcaoCode("KLM")))
        error <- repo.save(Aircraft(Registration("PH-BHA"), "B789", "Boeing 787-9", IcaoCode("KLM"))).flip
    ...
    test("delete removes an existing aircraft") {
      for
        ...
        repo  <- ZIO.service[AircraftRepository]
        _     <- repo.save(Aircraft(Registration("HB-JNA"), "A343", "Airbus A340-300", IcaoCode("SWR")))
