...
private[application] object RouteAirlineRepositoryStub:
  ...
  val unimplementedRouteAirlineRepo: RouteAirlineRepository = new RouteAirlineRepository:
    def associate(o: IataCode, d: IataCode, icao: IcaoCode): IO[DomainError, Unit]    =
      ZIO.die(new NotImplementedError("associate"))
    def disassociate(o: IataCode, d: IataCode, icao: IcaoCode): IO[DomainError, Unit] =
    ...
    def findRoutes(icao: IcaoCode, p: Pagination): IO[DomainError, List[Route]]       =
      ZIO.die(new NotImplementedError("findRoutes"))

  def stubRouteAirlineRepo(
      onAssociate: (IataCode, IataCode, IcaoCode) => IO[DomainError, Unit] =
        unimplementedRouteAirlineRepo.associate,
      onDisassociate: (IataCode, IataCode, IcaoCode) => IO[DomainError, Unit] =
      ...
      onFindRoutes: (IcaoCode, Pagination) => IO[DomainError, List[Route]] =
        unimplementedRouteAirlineRepo.findRoutes
  ): RouteAirlineRepository = new RouteAirlineRepository:
    def associate(o: IataCode, d: IataCode, icao: IcaoCode): IO[DomainError, Unit]    = onAssociate(o, d, icao)
    def disassociate(o: IataCode, d: IataCode, icao: IcaoCode): IO[DomainError, Unit] = onDisassociate(o, d, icao)
    def findAirlines(o: IataCode, d: IataCode): IO[DomainError, List[Airline]]        = onFindAirlines(o, d)
    def findRoutes(icao: IcaoCode, p: Pagination): IO[DomainError, List[Route]]       = onFindRoutes(icao, p)
