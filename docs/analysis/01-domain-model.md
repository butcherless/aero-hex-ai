...
| BR-12 | List pagination: `page ≥ 1`; `pageSize` between 1 and 100. | Enforced at the HTTP layer with `Validator.min/max` on every paginated endpoint (`CountryEndpoints.findAll`, `AirportEndpoints.findAll`/`findByCountry`). `Pagination.apply` (`shared-kernel/Pagination.scala:9-10`) additionally *clamps* silently as a domain-level fallback, so the HTTP 400 rejection is the actually-observed behavior for these three endpoints; the clamp only matters for any caller that bypasses HTTP validation. `[ASSUMPTION]` whether silent clamping vs. HTTP 400 rejection is the intended behavior for endpoints not yet covered (Airline/Aircraft/Flight/FlightInstance `findAll` still have no pagination validator). |
