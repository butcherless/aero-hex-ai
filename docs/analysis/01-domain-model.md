...
| `Airline` | Entity (Aggregate Root) | `AirlineIcaoCode` (natural key) | `icao: AirlineIcaoCode`, `name`, `foundationDate: LocalDate` — no `countryCode` field, same relationship-only pattern as `Airport`; `AirlineRepository.save`/`update` take it as a separate parameter | `domain/model/Airline.scala` |
