...
| `IcaoCode` | Value Object (opaque `String`) | — | No format validation in the type itself; format enforced only at the HTTP boundary (§5 BR-03), same pattern as `IataCode` | `domain/model/Airline.scala:1-8` |
...
| BR-04 | An Airport's/Airline's Country must already exist (referential integrity). | Enforced twice: application-observable via `QuillAirportRepository.resolveCountryId`/`QuillAirlineRepository.resolveCountryId` → `DomainError.CountryNotFound` (`infrastructure/persistence-quill/.../QuillAirportRepository.scala:35-44`, `QuillAirlineRepository.scala`), and at the DB level via FK `airports.country_id → countries.id` / `airlines.country_id → countries.id` (`V7__add_surrogate_keys.sql`). Both Quill and Doobie adapters enforce this for both entities (`DoobieAirlineRepository.resolveCountryId`, `sqlstate.class23.FOREIGN_KEY_VIOLATION` mapping). |
| BR-05 | An Airport's IATA code / an Airline's ICAO code must be unique. | Application pre-check in `CreateAirportService.create`/`CreateAirlineService.create` (`findByIata`/`findByIcao` → fail `AirportAlreadyExists`/`AirlineAlreadyExists` if present, `application/.../CreateAirportService.scala:12-15`, `CreateAirlineService.scala`), *and* DB `UNIQUE` on `airports.iata_code`/`airlines.icao_code` with SQLState `23505` mapped back to the same errors in both `QuillAirportRepository.save`/`QuillAirlineRepository.save` and `DoobieAirportRepository.save`/`DoobieAirlineRepository.save` (`infrastructure/persistence-quill/.../QuillAirportRepository.scala:98-114`). |
