...
| Outbox Event | A durable record of a domain event pending publication to Kafka, part of the transactional outbox pattern (`domain/outbox/OutboxEvent.scala`). |
...
|---|---|---|---|---|
| `Country` | Entity (Aggregate Root) | `CountryCode` (natural key) | `code: CountryCode`, `name: String` (no blank check in the domain type itself) | `domain/country/Country.scala` |
| `CountryCode` | Value Object — ZIO Prelude `Newtype[String]` (`IataCode`/`AirportIcaoCode`/`AirlineIcaoCode`/`Registration`/`FlightCode` were later migrated to the same pattern; `FlightInstanceId`/`OutboxEventId` remain plain Scala 3 opaque types) | — | `assertion = matches("^[a-zA-Z]{2}$".r)` enforces BR-01 at construction. `apply` validates compile-time literals (a malformed literal fails to *compile*); `make` validates runtime strings, returning `Validation[String, CountryCode]` bridged to `IO[DomainError, CountryCode]` via `.toZIO` (`CreateCountryRequest.toCommand`); `unsafeMake` bypasses validation for already-trusted data (DB reads, HTTP path params already Tapir-validated) | `domain/country/Country.scala:29-40` |
| `Airport` | Entity (Aggregate Root) | `IataCode` (natural key) | `iataCode: IataCode`, `icaoCode: AirportIcaoCode` (own type — split from a formerly-shared `IcaoCode`, see BR-03), `name`, `city` — no `countryCode` field; resolved by relationship, `AirportRepository.save`/`update` take it as a separate parameter | `domain/airport/Airport.scala` |
| `IataCode` | Value Object — ZIO Prelude `Newtype[String]` (migrated from a plain opaque type) | — | `assertion = matches("^[a-zA-Z]{3}$".r)` enforces BR-02 at construction. `CreateAirportRequest.toCommand` calls `IataCode.make(req.iata).toZIO`, failing with `DomainError.InvalidIataCode` (400); the Tapir-level `Validator.pattern` was removed once this landed (kept `minLength`/`maxLength`). `Route.origin`/`Route.destination` (cross-entity references, not `Airport` itself) still construct via `unsafeMake`, no real check on those fields | `domain/airport/Airport.scala:23-34` |
| `Airline` | Entity (Aggregate Root) | `AirlineIcaoCode` (natural key) | `icao: AirlineIcaoCode`, `name`, `alias: Option[String]`, `callsign: Option[String]`, `iata: Option[String]` — no `countryCode` field, same relationship-only pattern as `Airport`; `AirlineRepository.save`/`update` take it as a separate parameter. `foundationDate: LocalDate` was removed (see §6) — the entity has no source of founding-date data, so it was dropped rather than worked around; `alias`/`callsign` were added instead, both real fields the OpenFlights master-data source (`docs/todo/master-data/analysis.md` §2.3) actually provides. `iata` was added later, same rationale: OpenFlights' column 4 was always present in the source but unparsed until then — plain `Option[String]`, no shape rule, since IATA designators are alphanumeric (e.g. `"9W"`, `"6E"`), unlike `icao`'s letters-only pattern, and nothing looks an airline up *by* this field | `domain/airline/Airline.scala` |
| `AirportIcaoCode` | Value Object — ZIO Prelude `Newtype[String]` (split from a formerly-shared `IcaoCode`) | — | `assertion = matches("^[a-zA-Z]{4}$".r)` — exactly 4 letters, both shape *and* length now enforced by this type alone (BR-03). `CreateAirportRequest.toCommand` calls `AirportIcaoCode.make(...).toZIO`, failing with `DomainError.InvalidAirportIcaoCode` (400). Update paths still construct via `unsafeMake`, no real check | `domain/airport/Airport.scala:53-64` |
| `AirlineIcaoCode` | Value Object — ZIO Prelude `Newtype[String]` (split from a formerly-shared `IcaoCode`) | — | `assertion = matches("^[a-zA-Z]{3}$".r)` — exactly 3 letters, both shape *and* length now enforced by this type alone (BR-03). `CreateAirlineRequest.toCommand` calls `AirlineIcaoCode.make(...).toZIO`, failing with `DomainError.InvalidAirlineIcaoCode` (400). Update paths and every cross-entity reference field (`Route.airlineIcao`, `Flight.airlineIcao`, `Aircraft.airlineIcao`) still construct via `unsafeMake`, no real check | `domain/airline/Airline.scala:27-38` |
| `Aircraft` | Entity | `Registration` (natural key) | `registration: Registration`, `typeCode: String`, `description: String` (common/marketing name, e.g. "Airbus A330-900"), `airlineIcao: AirlineIcaoCode` (FK, constructed via `AirlineIcaoCode.unsafeMake` — a cross-entity reference, not `Aircraft`'s own natural key) — unlike `Airport`/`Airline`'s relationship to `Country`, this FK is a field on the entity itself, not a separate `save`/`update` parameter | `domain/aircraft/Aircraft.scala` |
| `Registration` | Value Object — ZIO Prelude `Newtype[String]` (migrated from a plain opaque type) | — | `assertion = matches("^.{1,10}$".r)` — non-blank plus a maximum length of 10, the same bound the HTTP boundary already validated; unlike `IataCode`/`AirportIcaoCode`/`AirlineIcaoCode`, no fixed alphabetic-shape pattern applies (real-world registrations vary by country of registry), so this assertion is bound-for-bound identical to the HTTP `Validator.minLength(1)`/`maxLength(10)` still kept alongside it — there is no input the domain check rejects that Tapir doesn't already reject. `CreateAircraftRequest.toCommand` calls `Registration.make(req.registration).toZIO`, failing with `DomainError.InvalidRegistration` (400) | `domain/aircraft/Aircraft.scala:28-39` |
| `Route` | Entity (Aggregate Root) | `(origin, destination): (IataCode, IataCode)` (natural key pair) | `origin`/`destination: IataCode` (FK, must differ — BR-07), `distanceKm: Int` (must be positive — BR-08; the IATA-standard measure for this is Great Circle Distance, GCD). No `id`/`airlineIcao` field — the surrogate id (`routes.id`, DB-generated UUID) is persistence-only, same convention as Country/Airport/Airline/Aircraft; the airline relationship moved to the `route_airlines` join table (BR-17) since it's many-to-many, not 1:1 | `domain/route/Route.scala` |
| `Flight` | Entity (Aggregate Root) | `FlightCode` (natural key) | `code: FlightCode`, `alias: Option[String]` (no format validation, plain `String`), `schedDeparture`/`schedArrival: LocalTime` (STD/STA — Scheduled Time of Departure/Arrival, in SSIM terms; no ordering constraint between them — an overnight flight's local arrival can precede its local departure), `origin`/`destination: IataCode` (FK to the Route segment, replacing the former `routeId: RouteId`), `airlineIcao: AirlineIcaoCode` (FK — pins which of the route's several operating airlines runs this specific Flight; no longer redundant now that `Route` itself carries no airline, see §7) | `domain/flight/Flight.scala` |
| `FlightCode` | Value Object — ZIO Prelude `Newtype[String]` (migrated from a plain opaque type) | — | `assertion = matches("^.{1,8}$".r)` — non-blank plus a maximum length of 8 (covers codeshare aliases like `"AEA9117"`); no alphabetic-shape pattern, mirroring `Registration`'s rationale exactly (real-world flight designators vary across codeshares/charters). `CreateFlightRequest.toCommand` calls `FlightCode.make(req.code).toZIO`, failing with `DomainError.InvalidFlightCode` (400) | `domain/flight/Flight.scala:28-39` |
| `FlightInstance` | Entity | `FlightInstanceId` (UUID, surrogate) | `departureDate`/`arrivalDate: LocalDateTime`, `flightCode: FlightCode` (FK), `registration: Registration` (FK) | `domain/flight/FlightInstance.scala` |
| `FlightInstanceId` | Value Object (opaque `UUID`) | — | `generate` factory | `domain/flight/FlightInstance.scala:7-13` |
| `OutboxEvent` | Entity | `OutboxEventId` (UUID) | `aggregateType`/`aggregateId`/`eventType`/`payload: String` (JSON serialized as plain `String`, stored as `JSONB`), `published: Boolean` | `domain/outbox/OutboxEvent.scala` |
...
| BR-07 | A Route's origin and destination airports must differ. | `RouteValidator.validate` (`domain/route/RouteValidator.scala:12-13`) → `DomainError.InvalidRoute`. |
| BR-08 | A Route's distance must be a positive number. | `RouteValidator.validate` (`domain/route/RouteValidator.scala:14-15`) → `InvalidRoute`; also DB `CHECK (distance_km > 0)` (`V4__create_routes.sql`). Note unit mismatch with the incubator source's "nautical miles" description — see §7. |
...
| BR-15 | An Aircraft's registration has no fixed format — only non-blank plus a maximum length of 10 characters are enforced, deliberately unlike the fixed-length IATA/ICAO codes governed by BR-01/02/03. | Both layers, same bounds: **domain layer**, `Registration`'s own `Newtype` assertion (`^.{1,10}$`, `domain/aircraft/Aircraft.scala`) — `CreateAircraftRequest.toCommand` calls `Registration.make(req.registration).toZIO`, failing with `DomainError.InvalidRegistration` (400); *and* HTTP layer, `AircraftEndpoints.scala`'s `registrationParam` path validator (`Validator.minLength(1)`/`maxLength(10)`) and `AircraftDto.scala`'s `CreateAircraftRequest` (same bounds) — kept deliberately, unlike `IataCode`/`AirportIcaoCode`/`AirlineIcaoCode`'s pattern validators, since there's no shape/pattern component to make one layer redundant. Real-world registration marks vary by country of registry (`"EC-MIG"` Spain, `"N12345"` US, `"G-ABCD"` UK) — a `[DECISION]`, not an oversight, to not attempt a single universal pattern. Reviewed again alongside the `IcaoCode` split (BR-03): the ≤10-char bound still comfortably covers all real-world registries, so no change was needed. `FlightCode`'s ≤8-char bound (BR-18) was reviewed the same way and reached the same conclusion. |
| BR-16 | A new Country's code must be a real ISO 3166-1 alpha-2 code, not just any 2 alphabetic string (BR-01 checks shape only, not membership). | `CreateCountryService.create` (`application/.../CreateCountryService.scala`) calls `CountryCode.validateIso` **before** the existing `AlreadyExists` check, via `ZIO.fromEither(CountryCode.validateIso(command.code)) *> ...`. `validateIso` returns `Either[DomainError, Unit]` — a pure, in-memory check against `IsoCountryCodes.all` (`domain/country/IsoCountryCodes.scala`, all 249 current ISO 3166-1 alpha-2 codes) — it fails with `DomainError.InvalidCountryCode` (400) itself if the code isn't recognized, rather than returning a `Boolean` for the caller to interpret, matching how `CountryRepository` already owns `CountryAlreadyExists`/`CountryNotFound` construction for `save`/`update`/`delete`. Originally implemented as a DB-backed `CountryRepository.validateCode` against a standalone `country_codes` table (`V12__create_country_codes.sql`); moved into the domain layer since the ISO list is static reference data (changes on the order of years, not at runtime), unlike BR-04's check against this app's own dynamic `countries` table — `V19__drop_country_codes.sql` retired the table. This is the referential/existence half of `docs/analysis/validation-analysis-hexagonal.md`'s recommended split (§2, "B2"), implemented as a pure domain-service-style function rather than via a separate class (like `RouteValidator`). `update`/`delete` don't call it — a code that's already a `countries` row was necessarily valid at creation time. |
...
| Country code (membership, BR-16) | Must be one of the 249 real ISO 3166-1 alpha-2 codes, checked on create only | `domain/country/IsoCountryCodes.scala` (pure in-memory `Set`), `CountryCode.isRecognized`/`validateIso` — formerly `V12__create_country_codes.sql`'s standalone `country_codes` table, dropped in `V19` |
...
| Airport ICAO code | exactly 4 alphabetic chars, shape and length both domain-enforced | `V6__add_airport_icao_code.sql` (`VARCHAR(4)`), `AirportIcaoCode` Newtype (own field only, create path — BR-03), HTTP validators |
| Airline ICAO code | exactly 3 alphabetic chars, shape and length both domain-enforced | `V3__create_airlines.sql` (`VARCHAR(3)`), `AirlineIcaoCode` Newtype (own field only, create path — BR-03), HTTP validators |
| Airline alias / callsign | Both optional, no shape rule (`VARCHAR(100)`, nullable) | `V15__redesign_airline_columns.sql` — replaces the removed `foundation_date` column (`V9__add_airline_foundation_date.sql`), which had no data source to fill it from |
| Airline IATA designator | Optional, ≤ 2 chars, no shape rule, nullable | `V18__add_airline_iata.sql` — `iata_code VARCHAR(2)`; source column existed in OpenFlights data from the start (`docs/todo/master-data/analysis.md` §2.3) but was unparsed until this migration |
...
| Route distance | Positive integer, unit is **kilometres** per code (`distanceKm`, `RouteDto` description "Flight distance in kilometres"); IATA distance tables typically express this as Great Circle Distance (GCD) | `domain/route/Route.scala`, `V4__create_routes.sql` (`CHECK (distance_km > 0)`) — see §7 for a unit discrepancy against the incubator source, which describes the equivalent field in nautical miles |
...
`V16__create_users.sql`/`V17__create_revoked_tokens.sql` are deliberately not in the table above —
they back JWT authentication (`domain/user/`), a separate bounded context from the Aviation
Network modeled here (per `CLAUDE.md`: `User` is "authentication only, not part of the aviation
business domain"). See `docs/todo/auth-jwt.md` for that context's own model/roadmap.
...
3. **`RESOLVED`** ~~`Flight.airlineIcao` vs. `Route.airlineIcao`~~ — `Route` no longer carries an
   ...
   specific airline, among the route's several operators, to this scheduled Flight. Flight now has
   full CRUD (`CreateFlightUseCase`/`CreateFlightService`, plus update/delete — BR-18/BR-19), so
   this is exercised on every `POST /api/v1/flights` call, not just theoretically.
4. **`PARTIALLY RESOLVED`** ~~`CountryCode.from` and `NonEmptyString` are dead code~~ —
   ...
   `IcaoCode` (BR-03 — since split into `AirportIcaoCode`/`AirlineIcaoCode`, each now enforcing
   both shape *and* length, not just shape), and `Registration` (BR-15) — each entity's own
   natural-key field now gets a real `.make(...).toZIO` check on its create path
   (`CreateAirportRequest`, `CreateAirlineRequest`, `CreateAircraftRequest`), with the redundant
   Tapir `Validator.pattern` removed wherever the domain check made it dead code (`IataCode`;
   `Registration` had none to remove — see BR-15). Cross-entity reference fields (`Route`/`Flight`/`Aircraft`'s
...
6. **`RESOLVED`** ~~Airline has no write use cases~~ — `CreateAirlineUseCase`/`UpdateAirlineUseCase`/
   `DeleteAirlineUseCase` were added (`domain/airline/{Create,Update,Delete}AirlineUseCase.scala`),
   wired through `application/.../{Create,Update,Delete}AirlineService.scala` and
   `AirlineEndpoints.create/update/delete`, backed by Quill (`AirlineRepository.save`/
